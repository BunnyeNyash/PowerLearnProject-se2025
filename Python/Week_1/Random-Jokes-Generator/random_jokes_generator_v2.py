...
def main():
    ...
    while True:
        try:
            print("\nWelcome to the Random Jokes Generator App")
            input("Press Enter to hear a joke ... ")
            print("\n" + get_random_joke() + "\n")


            another_joke = input("\nWould you like to hear another joke? (yes or no) : ").strip().lower()
            positive_responses = ["yes", "yeah", "yeahp", "sure", "yep", "ok", "y", "okay", "absolutely", "why not", "why not?"]
            if another_joke not in positive_responses:
                print("\nThank You for using the Random Jokes Generator. Goodbye!!")
                break
